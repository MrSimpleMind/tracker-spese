function EditReminderModal({ reminder, onClose }) {
    const [titolo, setTitolo] = React.useState(reminder.titolo);
    const [descrizione, setDescrizione] = React.useState(reminder.descrizione || '');
    const [dataScadenza, setDataScadenza] = React.useState(reminder.dataScadenza);
    const [importoStimato, setImportoStimato] = React.useState(reminder.importoStimato || '');
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-end sm:items-center justify-center z-50">
            <div className="bg-white rounded-t-2xl sm:rounded-2xl w-full sm:max-w-lg">
                ...
                <form onSubmit={handleSubmit} className="p-4 space-y-4">
                    ...
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Importo Stimato (€) - Opzionale</label>
                        <input
                            type="number"
                            step="0.01"
